...
describe("C3Router", function () {
    ...
    async function deploy() {
        ...
        c3DappManager = await C3DappManager.deploy(_owner.address);
    ...
    describe("C3DappManager", function () {
        it("Deploy", async function () {
            expect(await c3DappManager.owner()).to.equal(owner.address);
        });
        it("setFeeCurrencies", async function () {
            await expect(c3DappManager.setFeeCurrencies([weth.target], [100000000], [1000000]))
                .to.emit(c3DappManager, "SetFeeConfig").withArgs(weth.target, "0", 100000000, 1000000);

            await expect(c3DappManager.disableFeeCurrency(weth.target))
                .to.emit(c3DappManager, "SetFeeConfig").withArgs(weth.target, "0", 0, 0);
        });


        it("setSpeFeeConfigByChain", async function () {
            await expect(c3DappManager.setSpeFeeConfigByChain(weth.target, "1", "200000000", "2000000"))
                .to.emit(c3DappManager, "SetFeeConfig").withArgs(weth.target, "1", 200000000, 2000000);
        });
        ...
        it("deposit and withdraw", async function () {
            await c3DappManager.setFeeCurrencies([weth.target, erc20Token.target], [100000000, 2000000000], [1000000, 20000000])
            let dappID = await c3DappManager.dappID();
            let nextID = new BN(dappID).add(new BN(1)).toString()
