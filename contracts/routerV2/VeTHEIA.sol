...
import {IStagingVault} from "./IStagingVault.sol";
 ...
 * @author Modified for Theia by @Selqui
...
interface IVotingEscrow is IERC721Metadata, IVotes {
    ...
    function setUp(address gov, address stagingVault, address theiaRewards, address treasury) external;
...
contract VotingEscrow is UUPSUpgradeable, IVotingEscrow {
    ...
    address public stagingVault;
    address public rewards;
    address public theiaRouter;
    ...
    // mapping from tokenId to liquidity contract address to chainId to amount
    mapping(uint256 => mapping(address => mapping(uint256 => uint256))) public liquidityProvided;
    ...
    string public constant name = "Voting Escrow Theia";
    string public constant symbol = "veTHEIA";
    string public constant version = "1.0.0";
    uint8 public constant decimals = 18;
    bytes16 internal constant HEX_DIGITS = "0123456789abcdef";
    uint256 internal constant WEEK = 1 weeks;
    uint256 internal constant MAXTIME = (365 * 86400)/2;  // 6 months
    uint256 internal constant MULTIPLIER = 1 ether;
    int128 internal constant iMAXTIME = (365 * 86400)/2;
    // penalty for liquidating veTHEIA before maturity date. 50,000 / 100,000  =  50%
    ...
    modifier checkNoRewards(uint256 _tokenId) {
        require(ITheiaRewards(rewards).unclaimedRewards(_tokenId) == 0, "veTHEIA: Claim rewards before interacting with token ID.");
        _;
    }

    modifier checkNoStaked(uint256 _tokenId) {
        (,, bool staked) = IStagingVault(stagingVault).getStakedAllChains(tokenId);
        require(!staked, "veTHEIA: tokenId still has some staked liquidity");
        _;
    }
    ...
    function split(uint256 _tokenId, uint256 _extraction) external 
        checkNoRewards(_tokenId) 
        checkNoStaked(_tokenId)
        returns (uint256) {
        _checkApprovedOrOwner(msg.sender, _tokenId);
    ...
    function setUp(
        ...
        stagingVault = _stagingVault == address(0) ? stagingVault : _stagingVault;
    ...
    function _checkpoint(uint256 _tokenId, LockedBalance memory old_locked, LockedBalance memory new_locked) internal {
        ...
        Point memory initial_last_point = last_point;// TODO bug need deep copy
